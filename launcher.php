		...
		if ($this->settings['subscribe']['service'] == 'feedburner') {
			...
			<form id="wplauncher-subscribe" method="post" novalidate="true">
				<fieldset class="wplauncher-subscribe-fieldset">
					<?php if ( $this->settings['subscribe']['include_name'] ) { ?>
						<input id="wplauncher-subscribe-first-name" name="wplauncher-fname" type="text" placeholder="<?php echo $this->settings['subscribe']['name_label']; ?>"/>
					<?php } ?>

					<?php if ( $this->settings['subscribe']['include_last_name'] ) { ?>
						<input id="wplauncher-subscribe-last-name" name="wplauncher-lname" type="text" placeholder="<?php echo $this->settings['subscribe']['last_name_label']; ?>"/>
					<?php } ?>
			...
			jQuery(document).ready(function($) {
				$('#wplauncher-subscribe').submit(function(event){
					var type = $('#wplauncher-subscribe-service').html();
				    if (/*$('#wplauncher-subscribe-service').html() == 'WordPress' ||*/
				    	type == 'aweber' ||
				    	type == 'mailchimp' ||
				    	type == 'getresponse' ||
						...
						$('<i style="margin-left: 10px;" class="wplauncher-subscribe-submit-spinner wplauncher-icon-spin animate-spin"></i>').insertAfter('#wplauncher-subscribe-submit');
						...
						$.post(ajaxurl, data, function(response) {
							...
							$('.wplauncher-subscribe-submit-spinner').remove();
							var icon = (response.status == 'check' ? 'ok' : 'cancel');
							$('.wplauncher-subscribe-message').html('<i class="wplauncher-icon-' + response.status + '"></i> ' + response.message);
